use spookyhash::{hash32, hash128, SpookyHash};

fn test_results() {
    println!("\ntesting results ...");
    const BUFSIZE: usize = 512;
    #[rustfmt::skip]
    let expected: [u32; BUFSIZE] = [
    ...
    for i in 0..BUFSIZE {
        buf[i] = (i + 128) as u8;
        let saw = hash32(&buf[..i], 0);
        if saw != expected[i] {
            println!("{:3}: saw 0x{:08x}, expected 0x{:08x}", i, saw, expected[i]);
            ok = false;
        }
    }
    if ok {
...
// Verify that hashing in pieces matches hashing in one call.
fn test_pieces() {
    println!("\ntesting pieces ...");
    ...
    for i in 0..BUFSIZE {
        ...
        let mut a = seed1;
        let mut b = seed2;
        hash128(&buf[..i], &mut a, &mut b);

        // all as one piece via streaming API
        state.init(seed1, seed2);
        state.update(&buf[..i]);
        let (c, d) = state.final_hash();
        if a != c {
            println!("wrong hash1 {}: {:016x} {:016x}", i, a, c);
            ok = false;
        }
        if b != d {
            println!("wrong hash2 {}: {:016x} {:016x}", i, b, d);
            ok = false;
        }
        ...
        for j in 0..i {
            state.init(seed1, seed2);
            state.update(&buf[..j]);
            state.update(&buf[j..i]);
            let (c2, d2) = state.final_hash();
            if a != c2 {
                println!("wrong hash1 {} {}: {:016x} {:016x}", j, i, a, c2);
                ok = false;
            }
            if b != d2 {
                println!("wrong hash2 {} {}: {:016x} {:016x}", j, i, b, d2);
                ok = false;
            }
        }
    }
